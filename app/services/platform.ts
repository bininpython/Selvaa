...
export async function loadCommunity() {
  ...
  const [groupsResult, eventsResult] = await Promise.all([
    ...
    supabase.from("events").select("id,name,description,start_at,meeting_point,difficulty,distance,max_participants,group_id").gte("start_at", new Date().toISOString()).order("start_at").limit(12),
  ]);
  if (groupsResult.error) throw new Error(`Falha ao carregar grupos: ${groupsResult.error.message}`);
  if (eventsResult.error) throw new Error(`Falha ao carregar eventos: ${eventsResult.error.message}`);
  return { groups: groupsResult.data as AdventureGroup[], events: eventsResult.data as OutdoorEvent[] };
}
